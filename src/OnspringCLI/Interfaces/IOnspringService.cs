namespace OnspringCLI.Interfaces
{
  public interface IOnspringService
  {
    ...
    Task<GetPagedRecordsResponse?> GetAPageOfRecords(string apiKey, int appId, List<int> fieldIds, PagingRequest pagingRequest);
    Task<GetFileResponse?> GetFile(string apiKey, OnspringFileRequest fileRequest);
    Task<ReportData?> GetReport(string apiKey, int reportId);
    Task<Field?> GetField(string apiKey, int fieldId);
    Task<GetPagedRecordsResponse?> GetAPageOfRecordsByQuery(string apiKey, int appId, List<int> fieldIds, string queryFilter, PagingRequest? pagingRequest = null);
