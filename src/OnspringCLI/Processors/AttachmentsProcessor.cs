...
class AttachmentsProcessor : IAttachmentsProcessor
{
  ...
  public async Task<List<Field>> GetFileFields(int appId, List<int>? fieldsFilter = null)
  {
    var fields = await _onspringService.GetAllFields(_globalOptions.Value.SourceApiKey, appId);

    if (fieldsFilter is not null && fieldsFilter.Count is not 0)
    {
      fields = fields
        .Where(f => fieldsFilter.Contains(f.Id))
        .ToList();
    }
  ...
  public async Task<List<OnspringFileRequest>> GetFileRequests(int appId, List<Field> fileFields, List<int>? filesFilter = null, List<int>? recordsFilter = null)
  {
    ...
    _logger.Debug("Retrieving records whose information needs to be requested.");

    do
    {
      _logger.Debug("Retrieving records for page {PageNumber}.", currentPage);

      var res = await _onspringService.GetAPageOfRecords(_globalOptions.Value.SourceApiKey, appId, fileFieldIds, pagingRequest);

      if (res is null)
      {
        _logger.Warning("No records found for page {PageNumber}.", currentPage);
        break;
      }

      _logger.Debug("Records retrieved for page {PageNumber}. {Count} records found.", currentPage, res.Items.Count);

      totalPages = res.TotalPages;

      foreach (var record in res.Items)
      {
        if (recordsFilter is not null && recordsFilter.Count is not 0 && recordsFilter.Contains(record.RecordId) is false)
        {
          continue;
        }

        var requests = GetFileRequestsFromRecord(record, fileFields, filesFilter);

        fileRequests.AddRange(requests);
      }

      _logger.Debug("Retrieved files from page {CurrentPage} of records.", currentPage);

      pagingRequest.PageNumber++;
      currentPage = pagingRequest.PageNumber;
    } while (currentPage <= totalPages);

    _logger.Debug("Finished retrieving records whose information needs to be requested.");

    return fileRequests;
  }

  public async Task<List<OnspringFileInfoResult>> GetFileInfos(List<OnspringFileRequest> fileRequests)
  {
    var fileInfos = new ConcurrentBag<OnspringFileInfoResult>();

    _logger.Debug("Retrieving information for {Count} files.", fileRequests.Count);

    using var pBar = _progressBarFactory.Create(fileRequests.Count, "Starting to retrieve information for files.");
    ...
    _logger.Debug("Information retrieved for {Count} files.", fileInfos.Count);

    return [.. fileInfos];
  }

  public void WriteFileInfoReport(List<OnspringFileInfoResult> fileInfos, string outputDirectory
  )
  {
  ...
  public void WriteFileRequestErrorReport(List<OnspringFileRequest> fileRequests, string outputDirectory)
  {
    _reportService.WriteCsvReport(
      fileRequests,
  ...
  {
    var report = await _onspringService.GetReport(_globalOptions.Value.SourceApiKey, reportId);

    if (report is null)
    {
      _logger.Warning("Unable to get report {ReportId}.", reportId);
      return [];
    }
  ...
  public async Task<List<OnspringFileRequest>> TryDownloadFiles(List<OnspringFileRequest> fileRequests, string outputDirectory)
  {
    var outputPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, outputDirectory);

    Directory.CreateDirectory(outputPath);

    var fileName = Path.Combine(outputPath, "file-list.csv");
    ...
    using var pBar = _progressBarFactory.Create(fileRequests.Count, "Downloading files");
    _logLevelSwitch.MinimumLevel = LogEventLevel.Fatal;
    ...
    foreach (var fileRequest in fileRequests)
    {
      ...
      var file = await GetFile(fileRequest, outputDirectory);
  ...
  public async Task<List<OnspringFileRequest>> TryDeleteFiles(List<OnspringFileRequest> fileRequests)
  {
    using var pBar = _progressBarFactory.Create(fileRequests.Count, "Deleting files");
    _logLevelSwitch.MinimumLevel = LogEventLevel.Fatal;
    ...
    foreach (var fileRequest in fileRequests)
    {
      ...
      var isDeleted = await TryDeleteFile(fileRequest);
  ...
  public async Task<bool> ValidateMatchFields(IAttachmentTransferSettings settings)
  {
    var sourceMatchField = await _onspringService.GetField(_globalOptions.Value.SourceApiKey, settings.SourceMatchFieldId);
    var targetMatchField = await _onspringService.GetField(_globalOptions.Value.TargetApiKey, settings.TargetMatchFieldId);
  ...
  public async Task<bool> ValidateFlagFieldIdAndValues(IAttachmentTransferSettings settings)
  {
    var flagField = await _onspringService.GetField(_globalOptions.Value.SourceApiKey, settings.ProcessFlagFieldId);
    ...
    var processListValue = listField.Values
      .FirstOrDefault(
        v =>
          v.Name.Equals(processValue, StringComparison.CurrentCultureIgnoreCase) ||
          (Guid.TryParse(processValue, out var result) && v.Id == result)
      );

    var processedValue = settings.ProcessedFlagValue;

    var processedListValue = listField.Values
      .FirstOrDefault(
        v =>
          v.Name.Equals(processedValue, StringComparison.CurrentCultureIgnoreCase) ||
    ...
    settings.ProcessFlagListValueId = processListValue.Id;
    settings.ProcessedFlagListValueId = processedListValue.Id;
    return true;
  }

  public async Task<List<ResultRecord>> GetSourceRecordsToProcess(IAttachmentTransferSettings settings, List<int>? recordsFilter = null)
  {
    ...
    sourceFieldIds.AddRange(settings.AttachmentFieldIdMappings.Keys.ToList());
    ...
    _logger.Debug("Retrieving records whose attachments need to be transferred.");
    ...
    _logger.Debug("Finished retrieving records whose information needs to be transferred.");

    return sourceRecords;
  }

  public async Task TransferAttachments(IAttachmentTransferSettings settings, List<ResultRecord> sourceRecords)
  {
    _logger.Debug("Transferring attachments for {Count} records.", sourceRecords.Count);

    using var pBar = _progressBarFactory.Create(sourceRecords.Count, "Transferring attachments");
    _logLevelSwitch.MinimumLevel = LogEventLevel.Fatal;

    await Parallel.ForEachAsync(sourceRecords, async (sourceRecord, token) =>
    {
      pBar.Message = $"Transferring attachments for record {sourceRecord.RecordId}";

      await TransferRecordAttachments(settings, sourceRecord);

      pBar.Tick($"Transferred attachments for record {sourceRecord.RecordId}");

      _logger.Debug("Transferred attachments for record {RecordId}.", sourceRecord.RecordId);
    });

    pBar.Message = "Transferred attachments";
    _logLevelSwitch.MinimumLevel = LogEventLevel.Information;

    _logger.Debug("Finished transferring attachments for {Count} records.", sourceRecords.Count);
  }

  public async Task TransferRecordAttachments(IAttachmentTransferSettings settings, ResultRecord sourceRecord)
  {
    _logger.Debug(
    ...
    var matchValue = GetRecordFieldValue(sourceRecord, settings.SourceMatchFieldId);
    ...
    if (targetRecords.Items.Count > 1)
    {
      _logger.Warning(
    ...
    var sourceAttachmentFieldIds = settings.AttachmentFieldIdMappings.Keys.ToList();

    foreach (var sourceAttachmentFieldId in sourceAttachmentFieldIds)
    {
      var targetAttachmentFieldId = settings.AttachmentFieldIdMappings.GetValueOrDefault(sourceAttachmentFieldId);

      await TransferAttachmentsForField(sourceRecord, targetRecordId, sourceAttachmentFieldId, targetAttachmentFieldId);
    }
    ...
    var updateResponse = await _onspringService.UpdateRecord(_globalOptions.Value.SourceApiKey, sourceRecordUpdates);
  ...
  private async Task<bool> TryDeleteFile(OnspringFileRequest fileRequest)
  {
    _logger.Debug(
    ...
    var isDeleted = await _onspringService.TryDeleteFile(_globalOptions.Value.SourceApiKey, fileRequest);
  ...
  internal async Task<OnspringFileResult?> GetFile(OnspringFileRequest fileRequest, string outputDirectory)
  {
    _logger.Debug(
    ...
    var res = await _onspringService.GetFile(_globalOptions.Value.SourceApiKey, fileRequest);
  ...
  {
    var attachmentFieldValue = GetRecordFieldValue(sourceRecord, sourceAttachmentFieldId);
  ...
  )
  {
    ...
    var sourceFileInfo = await _onspringService.GetFileInfo(_globalOptions.Value.SourceApiKey, sourceFileRequest);
    ...
    var sourceFile = await _onspringService.GetFile(_globalOptions.Value.SourceApiKey, sourceFileRequest);
    ...
    var res = await _onspringService.SaveFile(_globalOptions.Value.TargetApiKey, saveFileRequest);
  ...
  internal static RecordFieldValue? GetRecordFieldValue(ResultRecord record, int fieldId)
  {
    return record.FieldData.FirstOrDefault(x => x.FieldId == fieldId);
  }

  internal async Task<OnspringFileInfoResult> GetFileInfo(OnspringFileRequest fileRequest)
  {
    _logger.Debug(
  ...
  internal static List<OnspringFileRequest> GetFileRequestsFromRecord(ResultRecord record, List<Field> fileFields, List<int>? filesFilter)
  {
    ...
    foreach (var fieldValue in record.FieldData)
    {
      ...
      if (field.Type is FieldType.Attachment)
      {
        ...
        {
          if (attachment.StorageLocation is not FileStorageSite.Internal)
          {
            continue;
          }
  ...
  internal static bool IsAllAttachmentsField(ResultRecord record, List<Field> fileFields, List<AttachmentFile> attachmentFieldValue)
  {
    ...
    if (containsAllFileIds is true && attachmentFieldIds.Count is 2)
    {
      return false;
    }
