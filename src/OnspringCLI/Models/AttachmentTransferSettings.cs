...
public class AttachmentTransferSettings : IAttachmentTransferSettings
{
  ...
  public Dictionary<string, int> AttachmentFieldMappings { get; set; } = [];
  public Dictionary<int, int> AttachmentFieldIdMappings => AttachmentFieldMappings.ToDictionary(kvp => int.TryParse(kvp.Key, out var id) ? id : 0, kvp => kvp.Value);
